...
#root.configure(bg='black') color fe fondo
...
def render_todos():
    ...
    for i in range(0, len(rows)):
        ...
        color = 'blue' if completed else '#000000'
        l= Checkbutton(frame,text=descripccion, fg=color, width=42, anchor='w', command=complete(id))
...
def addTodo():
    ...
    if todo:
     c.execute("""
            INSERT INTO todo (descripccion ,completed) VALUES (?, ?)
     """, (todo, False))
     conn.commit()
     e.delete(0,END)
     render_todos()
